...
이 문서는 플레이 엑스포 전시를 위해 구축한 **2.5D 사이버펑크 허브 씬(Hub Scene)**의 개발 완료 사항과 향후 확장할 수 있는 추가 추천 작업 목록을 정리한 로드맵입니다.
...
## 🚀 2. 앞으로 추가하면 좋은 추천 작업 (로드맵)

전시용 빌드의 퀄리티를 한 차원 더 끌어올리기 위한 추천 추가 작업 마일스톤입니다.

```mermaid
graph TD
    A[1. 저장 및 불러오기 연동] --> B[2. 오토바이 씬 조작 연동]
    B --> C[3. 홀로그램 연출 및 VFX 강화]
    C --> D[4. 다단계 미션 시스템 도입]
```
